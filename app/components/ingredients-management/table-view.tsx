					...
					<TableBody>
						{isFetching ? (
							...
							items.map((ingredient) => (
								<TableRow
									key={ingredient.id}
									className={`group cursor-pointer transition-colors ${ingredient.is_deleted ? 'opacity-50' : 'opacity-100'}`}
								...
								>
									...
									<TableCell>
										<div className='flex flex-wrap gap-1'>
											...
											{ingredient.categories.length > 3 && (
												...
												<Tooltip>
													<TooltipTrigger>
														<Button
															variant='ghost'
															size='icon'
															className='h-8 w-8 z-10 opacity-100'
															onClick={(e) => {
																e.stopPropagation();
