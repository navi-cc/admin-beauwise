...
interface IngredientAnalysisEditorProps {
...
export function IngredientAnalysisEditor({
	content,
	onChange,
	placeholder = 'Write your Ingredient Analysis prompt... Type {{ or /variable to insert runtime variables.',
	editable = true
}: IngredientAnalysisEditorProps) {
	...
	React.useEffect(() => {
		...
		if (ext) {
			ext.options.promptType = 'ingredient_analysis';
					...
					<div className='flex items-center justify-between px-4 py-2 border-t bg-muted/20 text-[10px] text-muted-foreground'>
						<div className='flex items-center gap-2'>
							<span className='uppercase tracking-wider font-semibold'>
								Variables ({existingVariables.length} / 21):
							</span>
